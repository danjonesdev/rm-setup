            ...
            <div key={i} className="link  w-100  zoom-in-fade-in-iteration--cont">
              <article className="col-sm-12  col-md-6  pv3  latestArticleList__col--latest">
                ...
                <figure className="rel  pb3">
                  <Link to={`/Article/${article.title.replace(/\s+/g, '-')}`} className="db  over-hidden  latestArticleList__img--cont">
                ...
                <Link to={`/Article/${article.title.replace(/\s+/g, '-')}`} className="no-underline"><h2 className="black  t7  pt2  cp  title-font  over-hidden  link  latestArticleList__title">{article.title}</h2></Link>
